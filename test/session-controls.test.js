...
test(
  "only the host can control a fully ready phone session",
  { timeout: 10_000 },
  async (t) => {
    ...
    await new Promise((resolve, reject) => {
      ...
      assert.equal(
        healthResponse.headers.get("referrer-policy"),
        "no-referrer",
      );
      ...
      const phoneJoined = waitForDevices(host, (devices) =>
        devices.some(
          (device) => device.label === "Phone" && device.role === "phone",
        ),
      );
      ...
      assert.deepEqual(
        await emitWithAck(host, "host:play-pulse", { id: phone.id }),
        { ok: true, label: "Phone" },
      );
      ...
      const rejoined = waitForDevices(host, (devices) => {
        ...
        return (
          matches.length === 1 && matches[0].connected && !matches[0].ready
        );
      });
      phone.emit("client:profile", { label: "Phone" });
      await rejoined;
